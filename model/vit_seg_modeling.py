...
class Embeddings(nn.Module):
    ...
    def forward(self, x):
        if self.hybrid:
            ...
            features = None
        #x = self.ema(x)  #befor CNN
        x = self.patch_embeddings(x)  # (B, hidden. n_patches^(1/2), n_patches^(1/2)) fenkuaibingqianru
        x = x.flatten(2)
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        #self.ema = EMA(in_channels + skip_channels)
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):  ##config is backbone
